...
var InputValueUI=class InputValueUIFactory{
	constructor(options){
...
class XMLTemplate{
	...
	createDataInit(source,wrapperId){
		if(source===undefined){
			source={rootFolder:this.rootFolder,containerId:this.containerId,parent:this};
		}
		return {source:source,rootFolder:this.rootFolder,containerId:typeof wrapperId==='undefined'?this.myWrapperId:wrapperId,parent:this};
	}
	createUI(){
...
class BodyPart extends BodyValue{
	...
	buildRepeat(number){
		...
		for(var i=curRepeat;i<(curRepeat+number);i++){
			...
			var repeat=this.childsRepeat[i]=this.cloneChilds(divId);
...
class RepeatCtrl extends CtrlView{
	constructor(options){
		super(options);
		var source=this.source;
...
class Template extends XMLTemplate{
	...
	buildChild(childSource,arrChilds){
		for(var i=0;i<childSource.length;i++){
			arrChilds.push(new ChildWrapper(this.createDataInit(childSource[i])));
		}
...
class ChildWrapper extends XMLTemplate{
	...
	generateInput(){
		...
		this.inputValueUI=new myModuleUI.InputValueUI({inputType:this.inputType,containerId:this.myWrapperId,viewId:this.inputViewId,value:this.values});
		// if(this.inputType.toUpperCase()=='TEXT'){
	...
	getValueInput(){
		...
		return this.inputValueUI.getValue();
	}
...
class TemplateFactory{
	...
	initRootTemplate(){
		...
		for(var i=0;i<templateXmlList.length;i++){
			var template=new RootTemplate({source:templateXmlList[i],rootFolder:this.getRootFolder(),containerId:this.containerId});
	...
	buildSelectEntity(){
		console.log('templates',this.templateMap);
